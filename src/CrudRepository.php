...
abstract class CrudRepository {
    ...
    static public $model = Model::class;
    ...
    public static function index(array $params = [], bool $paginate = false, $functionExtraParametersTreatment = null) {
        if(sizeof($params) > 0) {
            ...
            if($functionExtraParametersTreatment!=null) $functionExtraParametersTreatment($clause, $params);
                ...
                foreach ($params as $attr => $val) {
                    if($val!==null && $val!=='') {
                        if(str_contains($attr, '.')) {
                            ...
                            $attribute = $separate[sizeof($separate)-1];
        ...
        else if($functionExtraParametersTreatment!=null) {
            $clause = (static::$model)::query();
        ...
        else return $paginate ? (static::$model)::paginate() : (static::$model)::toSql();
    }
    ...
    public static function update($model, $params) {
        if(is_numeric($model)) $model = self::show($model);
        else if(is_string($model)) $model = self::showByUuid($model);
    ...
    public static function destroy($model, $functionExtraParametersTreatment = null) {
        ...
        if($functionExtraParametersTreatment!=null) $functionExtraParametersTreatment($model->id);
